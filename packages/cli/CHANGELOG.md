# @my-cli/cli

## 0.4.7

### Patch Changes

- Updated dependencies [c9756bd]
  - @chrisluyi/os@0.3.5

## 0.4.6

### Patch Changes

- Updated dependencies [44870c1]
  - @chrisluyi/os@0.3.4

## 0.4.5

### Patch Changes

- Updated dependencies [571be25]
  - @chrisluyi/os@0.3.3

## 0.4.4

### Patch Changes

- b75cb63: Fix: resolve workspace:\* dependencies to real semver ranges before npm publish.
- Updated dependencies [b75cb63]
  - @chrisluyi/git@0.4.1
  - @chrisluyi/os@0.3.2

## 0.4.3

### Patch Changes

- Updated dependencies [abdffbe]
  - @chrisluyi/os@0.3.1

## 0.4.2

### Patch Changes

- Updated dependencies [a5d33f0]
  - @chrisluyi/os@0.3.0

## 0.4.1

### Patch Changes

- e546a4f: Add @chrisluyi/os package with cos open, cos term, cos ports, cos kill, and cos info commands.
- Updated dependencies [e546a4f]
  - @chrisluyi/os@0.2.0

## 0.4.0

### Minor Changes

- 8c29015: Add `cgit v2` (create next version branch) and `cgit push` (push to origin with upstream set) commands.

### Patch Changes

- Updated dependencies [8c29015]
  - @chrisluyi/git@0.4.0

## 0.3.0

### Minor Changes

- e7a6b91: Rename npm scope from @my-cli to @chrisluyi.

### Patch Changes

- Updated dependencies [e7a6b91]
  - @chrisluyi/core@0.3.0
  - @chrisluyi/git@0.3.0

## 0.2.0

### Minor Changes

- 31725a3: Initial release — personal CLI toolkit with cgit git shortcuts (log, wip, branch, squash) and master help command.

### Patch Changes

- Updated dependencies [31725a3]
  - @my-cli/core@0.2.0
  - @my-cli/git@0.2.0
